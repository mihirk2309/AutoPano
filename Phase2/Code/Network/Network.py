...
class TensorDLT(torch.nn.Module):
    ...
    def forward(self, xA, xB):
        ...
        A = A + 1e-4*torch.randn(A.shape)

        try:
            _, S, V = torch.svd(A)
        except RuntimeError:
            warnings.warn('SVD did not converge', RuntimeWarning)
            return torch.empty((xA.size(0), 3, 3))
...
class STN(torch.nn.Module):
    def __init__(self):
        ...
        H = 240
        W = 320
        self.M = torch.tensor([[W/2, 0, W/2],[0, H/2, H/2],[0,0,1]]).float()
        self.M_inv = torch.inverse(self.M).float()

        self.affine_identity = torch.tensor([[1,0,0],[0,1,0]]).float()
    ...
    def forward(self, x, H):
        ...
        ones = torch.ones(grid.shape[0:3]).unsqueeze(-1)
...
if __name__ == '__main__':
    ...
    data = pickle.load(open("../Data/homography_data_val.pkl", "rb"))
