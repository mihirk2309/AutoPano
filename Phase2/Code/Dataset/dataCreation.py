...
class HomographyDataset(Dataset):
    ...
    def __init__(self, dirpath, generate=True, transform=None, name="train"):
        ...
        if generate:
            self.info = list()
            num = 4500 if name=="train" else 500
            for i in range(num):
                if i % 1000 == 0:
                    print("Completed ", i)
            ...
            pickle.dump(self.info, open(f"../Data/homography2_data_{name}.pkl", "wb"))
        
        else:
            self.info = pickle.load(open(f"../Data/homography2_data_{name}.pkl", "rb"))

        if name == "train":
           self.info = self.info[:1000]
...
if __name__ == '__main__':
    ...
    H = HomographyDataset('../Data/Train/', generate=True, transform=T, name="test")
    l = list()
